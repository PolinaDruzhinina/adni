...
from model import AutoEncoder, Conv5_FC3, transfer_autoencoder_weights, transfer_cnn_weights, load_model
from data import load_data, generate_sampler, get_transforms, MRIDatasetImage
from utils import return_logger, EarlyStopping, commandline_to_json, visualize_image
from test import test, save_checkpoint, test_single_cnn, test_ae
import wandb
...
def train_ae(decoder, train_loader, valid_loader, criterion, optimizer, resume,
    ...
    if logger is None:
        logger = logging
    wandb.watch(decoder, criterion, log="all", log_freq=5)
            ...
            if (i + 1) % options.accumulation_steps == 0:
                ...
                if options.evaluation_steps != 0 and (i + 1) % options.evaluation_steps == 0:
                    ...
                    wandb.log({"train": {"epoch": epoch,
                               ...
                               "test": {'loss': mean_loss_valid}}, step=i+epoch*len(train_loader))
...
def train_single_cnn(args):
    ...
    for fi in fold_iterator:
        ...
        if params.gpu:
            ...
            model.cpu()
        if params.network_type == 'autoencoder':
        ...
        elif params.network_type == 'autoencoder':
        ...
        if params.network_type == 'cnn':
            ...
            train_ae(model, train_loader, valid_loader, criterion,
              optimizer, False, log_dir, model_dir, params, train_logger)
